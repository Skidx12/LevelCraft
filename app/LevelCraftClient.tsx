...
import { BookOpen, Check, ChevronRight, Code2, Flame, Gauge, GitBranch, LockKeyhole, LogOut, Menu, Play, ShieldCheck, Sparkles, Swords, Target, Trophy, UserPlus, Users, Wrench, X, Zap } from "lucide-react";
...
type FriendProgress = { email: string; display_name?: string | null; track?: string | null; level?: string | null; xp?: number | null; checkpoint?: number | null; updated_at?: number | null };
...
function SideNav({ open, close }: { open: boolean; close: () => void }) {
  return <aside className={`sidebar ${open ? "sidebar-open" : ""}`}>
    ...
    <nav aria-label="Primary"><a className="active" href="#dashboard"><Gauge /> Command Center</a><a href="#path"><GitBranch /> Quest Path</a><a href="#missions"><Target /> Missions <em>3</em></a><a href="#guild"><Users /> Guild Map</a><a href="#skills"><Sparkles /> Skill Tree</a><a href="#trials"><Trophy /> Rank Trials</a></nav>
...
export default function LevelCraftClient({ user }: { user: { displayName: string; email: string } }) {
  ...
  const [friends, setFriends] = useState<FriendProgress[]>([]); const [friendEmail, setFriendEmail] = useState(""); const [loaded, setLoaded] = useState(false); const [syncStatus, setSyncStatus] = useState("Loading checkpoint…");
  ...
  const loadProgress = async () => {
    try { const response = await fetch("/api/progress", { cache: "no-store" }); const data = await response.json(); if (!response.ok) throw new Error();
      if (data.own) { setTrack(data.own.track); setLevel(data.own.level); setCompleted(data.own.completed || []); } setFriends(data.friends || []); setSyncStatus("Checkpoint synced");
    } catch { setSyncStatus("Sync unavailable — retry later"); } finally { setLoaded(true); }
  };
  useEffect(() => { void loadProgress(); }, []);
  useEffect(() => { if (!loaded) return; const timer = window.setTimeout(async () => { setSyncStatus("Saving checkpoint…"); try { const response = await fetch("/api/progress", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "save_progress", track, level, completed }) }); if (!response.ok) throw new Error(); setSyncStatus("Checkpoint synced"); } catch { setSyncStatus("Save failed — progress kept on screen"); } }, 500); return () => window.clearTimeout(timer); }, [track, level, completed, loaded]);
  useEffect(() => {
  ...
  const addFriend = async (event: React.FormEvent) => { event.preventDefault(); const email = friendEmail.trim(); if (!email) return; setSyncStatus("Adding guild member…"); const response = await fetch("/api/progress", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "add_friend", email }) }); const data = await response.json(); if (response.ok) { setFriendEmail(""); await loadProgress(); } else setSyncStatus(data.error || "Could not add friend"); };
  ...
  return <div className="app-shell"><SideNav open={navOpen} close={() => setNavOpen(false)} />{navOpen && <button className="scrim" onClick={() => setNavOpen(false)} aria-label="Close menu" />}
    <main id="dashboard" className="main-content">
      <header className="topbar"><button className="menu-btn" onClick={() => setNavOpen(true)} aria-label="Open navigation"><Menu /></button><div><p>HUNTER / {user.email}</p><h1>Command Center</h1></div><div className="top-actions"><span className="sync-status">{syncStatus}</span><button className="track-switch" onClick={() => setShowTracks(!showTracks)}><span>{tracks.find(t => t.name === track)?.icon}</span><div><small>ACTIVE PATH</small><b>{track}</b></div><ChevronRight /></button><a className="avatar" title={`${user.displayName} — Sign out`} href="/signout-with-chatgpt?return_to=/">{user.displayName.split(/\s|@/).slice(0,2).map(x=>x[0]).join("").toUpperCase()}<i /></a></div></header>
      ...
      <div className="dashboard-grid"><section className="primary-column"><div className="section-title" id="missions"><div><span>ACTIVE QUEST</span><h2>Your next mission</h2></div><small>{available.filter(m => completed.includes(m.id)).length}/{available.length} available missions cleared</small></div>
        ...
        <section className="guild-section" id="guild"><div className="guild-head"><div><span>GUILD EXPEDITION</span><h2>{track} checkpoint map</h2><p>See where your party last saved. Only friends you add are shown.</p></div><form onSubmit={addFriend}><input type="email" value={friendEmail} onChange={e => setFriendEmail(e.target.value)} placeholder="friend@email.com" aria-label="Friend email" required/><Button type="submit"><UserPlus /> Add friend</Button></form></div>
          <div className="arcade-map">{phases.map((phase,index) => { const checkpointFloor=index*3; const checkpointCeil=index===phases.length-1?99:(index+1)*3-1; const party=[{email:user.email,display_name:user.displayName,track,checkpoint:completed.length,xp},...friends].filter(f=>f.track===track && Number(f.checkpoint||0)>=checkpointFloor && Number(f.checkpoint||0)<=checkpointCeil); return <div className={`map-stage ${index <= activePhase ? "reached" : ""}`} key={phase.phase}><div className="map-line"/><div className="map-node"><span>{phase.rank}</span></div><b>{phase.phase.replace(" Gate","")}</b><small>Checkpoint {checkpointFloor}</small><div className="party-markers">{party.map((member,i)=><div className={`party-avatar ${member.email===user.email?"me":""}`} title={`${member.display_name||member.email} · ${member.xp||0} XP`} key={`${member.email}-${i}`}>{(member.display_name||member.email).split(/\s|@/).slice(0,2).map(x=>x[0]).join("").toUpperCase()}</div>)}</div></div>})}</div>
          {friends.length===0 ? <div className="guild-empty"><Users /><div><b>Your party is empty</b><span>Add a friend by the email they use for ChatGPT. Their marker appears after their first sign-in.</span></div></div> : <div className="guild-roster">{friends.map(friend=><div key={friend.email}><span className={friend.track?"online":"pending"}/><div><b>{friend.display_name||friend.email}</b><small>{friend.track ? `${friend.track} · ${friend.xp||0} XP · checkpoint ${friend.checkpoint||0}` : "Invite saved · waiting for first sign-in"}</small></div></div>)}</div>}
        </section>
      </section><aside className="right-column"><section className="panel mastery" id="skills"><div className="panel-title"><span>MASTERY MATRIX</span><b>{track}</b></div>{[["Core concepts",72],["Applied coding",48],["Testing & quality",34],["Production readiness",18]].map(([label,val]) => <div className="skill" key={label as string}><div><span>{label}</span><b>{val}%</b></div><Progress value={val as number} /></div>)}</section><section className="panel boss" id="trials"><div className="boss-icon"><Swords /></div><span>RANK TRIAL</span><h3>The Production Incident</h3><p>Unlock by clearing the Production Gate. Diagnose latency, data inconsistency and a failing dependency under a time limit.</p><div><LockKeyhole /> Requires Rank C</div></section><section className="panel principle"><BookOpen /><div><span>TODAY&apos;S PRINCIPLE</span><p>“Working code is the entry fee. Explainable, testable and operable code is the profession.”</p></div></section><a className="signout-card" href="/signout-with-chatgpt?return_to=/"><LogOut /> Sign out of LevelCraft</a></aside></div>
      <footer><span>LEVELCRAFT SYSTEM // LEARN → BUILD → BREAK → FIX → SHIP</span><span>Progress is securely saved to your account</span></footer>
    </main></div>;
}
